...
class Realm(models.Model):
    ...
    MAX_GOOGLE_HANGOUTS_DOMAIN_LENGTH = (
        255  # This is just the maximum domain length by RFC
    )
    ...
    _max_invites = models.IntegerField(
        null=True, db_column="max_invites"
    )  # type: Optional[int]
    disallow_disposable_email_addresses = models.BooleanField(
        default=True
    )  # type: bool
    authentication_methods = BitField(
    ...
    message_content_allowed_in_email_notifications = models.BooleanField(
        default=True
    )  # type: bool
    ...
    create_stream_policy = models.PositiveSmallIntegerField(
        default=CREATE_STREAM_POLICY_MEMBERS
    )  # type: int
    ...
    message_content_delete_limit_seconds = models.IntegerField(
    ...
    message_content_edit_limit_seconds = models.IntegerField(
    ...
    bot_creation_policy = models.PositiveSmallIntegerField(
    ...
    def get_admin_users_and_bots(self) -> Sequence["UserProfile"]:
        ...
        return UserProfile.objects.filter(
            realm=self, role=UserProfile.ROLE_REALM_ADMINISTRATOR, is_active=True
        )
...
class RealmEmoji(models.Model):
    ...
    def __str__(self) -> str:
        return "<RealmEmoji(%s): %s %s %s %s>" % (
            self.realm.string_id,
            self.id,
            self.name,
            self.deactivated,
            self.file_name,
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    role = models.PositiveSmallIntegerField(
        default=ROLE_MEMBER, db_index=True
    )  # type: int
    ...
    enable_stream_desktop_notifications = models.BooleanField(
        default=False
    )  # type: bool
    ...
    enable_stream_audible_notifications = models.BooleanField(
        default=False
    )  # type: bool
    ...
    pm_content_in_desktop_notifications = models.BooleanField(
        default=True
    )  # type: bool
    ...
    message_content_in_email_notifications = models.BooleanField(
        default=True
    )  # type: bool
    ...
    demote_inactive_streams = models.PositiveSmallIntegerField(
    ...
    def can_subscribe_other_users(self) -> bool:
        ...
        assert (
            self.realm.invite_to_stream_policy
...
class Stream(models.Model):
    ...
    stream_post_policy = models.PositiveSmallIntegerField(
...
def bulk_get_streams(realm: Realm, stream_names: STREAM_NAMES) -> Dict[str, Any]:
    def fetch_streams_by_name(stream_names: List[str]) -> Sequence[Stream]:
        ...
        return (
            get_active_streams(realm.id)
            .select_related()
            .extra(where=[where_clause], params=stream_names)
        )
...
class ArchivedMessage(AbstractMessage):
    ...
    archive_transaction = models.ForeignKey(
...
class Message(AbstractMessage):
    ...
    def need_to_render_content(
        ...
        return (
            ...
            or rendered_content_version < bugdown_version
        )
...
class Attachment(AbstractAttachment):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "messages": [
                {"id": m.id, "name": time.mktime(m.date_sent.timetuple()) * 1000}
...
class Subscription(models.Model):
    ...
    desktop_notifications = models.NullBooleanField(
        default=None
    )  # type: Optional[bool]
    audible_notifications = models.NullBooleanField(
        default=None
    )  # type: Optional[bool]
    ...
    wildcard_mentions_notify = models.NullBooleanField(
        default=None
    )  # type: Optional[bool]
...
class UserPresence(models.Model):
    ...
    def status_from_string(status: str) -> Optional[int]:
        if status == "active":
            status_val = (
                UserPresence.ACTIVE
            )  # type: Optional[int] # See https://github.com/python/mypy/issues/2611
        elif status == "idle":
...
class ScheduledMessage(models.Model):
    ...
    def __str__(self) -> str:
        display_recipient = get_display_recipient(self.recipient)
        return "<ScheduledMessage: %s %s %s %s>" % (
            display_recipient,
            self.subject,
            self.sender,
            self.scheduled_timestamp,
...
def check_valid_user_ids(
    realm_id: int, user_ids: List[int], allow_deactivated: bool = False
) -> Optional[str]:
...
class CustomProfileField(models.Model):
    ...
    }  # type: Dict[int, Validator]
    FIELD_CONVERTERS = {
        item[0]: item[3] for item in ALL_FIELD_TYPES
    }  # type: Dict[int, Callable[[Any], Any]]
    FIELD_TYPE_CHOICES = [
    ...
    def __str__(self) -> str:
        return "<CustomProfileField: %s %s %s %d>" % (
            self.realm,
            self.name,
            self.field_type,
            self.order,
...
class CustomProfileFieldValue(models.Model):
    ...
    field = models.ForeignKey(
        CustomProfileField, on_delete=CASCADE
    ...
    def __str__(self) -> str:
        return "<CustomProfileFieldValue: %s %s %s>" % (
            self.user_profile,
            self.field,
            self.value,
        )
